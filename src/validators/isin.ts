...
export const isin: ValidatorFactory = () => ({
  validate(input) {
    if (input.value === "") return { valid: true };
    const v = input.value.toUpperCase();
    if (!/^[A-Z]{2}[0-9A-Z]{10}$/.test(v)) return { valid: false };
    if (!COUNTRY_CODES.has(v.substring(0, 2))) return { valid: false };

    // Convert letters A=10, B=11 ... Z=35, then flatten all digits
    let converted = "";
    ...
    // Double every other digit (Luhn-like)
    ...
    for (let i = 0; i < digits.length; i++) {
      sum += Number.parseInt(digits.charAt(i), 10);
    }
    sum = (10 - (sum % 10)) % 10;
