...
function makeFileInput(files: Array<{ name: string; size: number; type: string }>, opts: Record<string, unknown> = {}): ValidatorInput {
...
describe('file', () => {
  ...
  it('valid when extension matches', () => {
    const input = makeFileInput([{ name: 'photo.jpg', size: 100, type: 'image/jpeg' }], { extension: 'jpg,png' });
    expect(v.validate(input)).toEqual({ valid: true });
  });

  it('invalid when extension does not match', () => {
    const input = makeFileInput([{ name: 'doc.pdf', size: 100, type: 'application/pdf' }], { extension: 'jpg,png' });
    expect(v.validate(input)).toEqual({ valid: false });
  });

  it('invalid when file exceeds maxSize', () => {
    const input = makeFileInput([{ name: 'big.jpg', size: 5_000_001, type: 'image/jpeg' }], { maxSize: 5_000_000 });
    expect(v.validate(input)).toEqual({ valid: false });
  });

  it('valid when file is within maxSize', () => {
    const input = makeFileInput([{ name: 'small.jpg', size: 100, type: 'image/jpeg' }], { maxSize: 5_000_000 });
    expect(v.validate(input)).toEqual({ valid: true });
  });
