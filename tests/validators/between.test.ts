...
describe('between', () => {
  ...
  it('invalid for boundary when inclusive=false', () => {
    expect(v.validate(makeInput('1', { min: 1, max: 10, inclusive: false }))).toEqual({ valid: false });
    expect(v.validate(makeInput('10', { min: 1, max: 10, inclusive: false }))).toEqual({ valid: false });
  });
