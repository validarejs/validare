...
describe('ip', () => {
  ...
  it('valid for full IPv6', () => {
    expect(v.validate(makeInput('2001:0db8:85a3:0000:0000:8a2e:0370:7334'))).toEqual({ valid: true });
  });
  ...
  it('ipv6=false rejects IPv6', () => {
    expect(v.validate(makeInput('2001:0db8:85a3:0000:0000:8a2e:0370:7334', { ipv6: false }))).toEqual({ valid: false });
  });
});
