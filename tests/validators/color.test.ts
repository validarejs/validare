...
describe("color", () => {
  ...
  it("accepts hex color #ffffff", () =>
    expect(color().validate(input("#ffffff")).valid).toBe(true));
  it("accepts rgb color", () => expect(color().validate(input("rgb(255,0,0)")).valid).toBe(true));
  it("accepts rgba color", () =>
    expect(color().validate(input("rgba(255,0,0,0.5)")).valid).toBe(true));
  it("accepts hsl color", () =>
    expect(color().validate(input("hsl(0,100%,50%)")).valid).toBe(true));
  it("accepts hsla color", () =>
    expect(color().validate(input("hsla(0,100%,50%,1)")).valid).toBe(true));
  ...
  it("rejects hex-only type when given rgb", () =>
    expect(color().validate(input("rgb(0,0,0)", "hex")).valid).toBe(false));
  it("accepts hex-only type with hex value", () =>
    expect(color().validate(input("#abc", "hex")).valid).toBe(true));
});
