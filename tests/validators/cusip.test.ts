...
describe("cusip", () => {
  ...
  it("accepts valid CUSIP digits only", () => expect(v.validate(inp("037833100")).valid).toBe(true));
  it("accepts valid CUSIP with letters", () => expect(v.validate(inp("38259P508")).valid).toBe(true));
  it("rejects wrong check digit", () => expect(v.validate(inp("037833101")).valid).toBe(false));
  it("rejects containing I (not allowed)", () => expect(v.validate(inp("03783310I")).valid).toBe(false));
